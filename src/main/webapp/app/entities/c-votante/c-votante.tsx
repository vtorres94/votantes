...
export interface ICVotanteProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}
...
export const CVotante = (props: ICVotanteProps) => {
  ...
  const { cVotanteList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="c-votante-heading" data-cy="CVotanteHeading">
        C Votantes
        <div className="d-flex justify-content-end">
          ...
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            ...
            &nbsp; Create new C Votante
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        {cVotanteList && cVotanteList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  Defensor Voto <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {cVotanteList.map((cVotante, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`${match.url}/${cVotante.id}`} color="link" size="sm">
                      {cVotante.id}
                    </Button>
                  </td>
                  <td>{cVotante.nombreCompleto}</td>
                  <td>{cVotante.nombre}</td>
                  <td>{cVotante.segundoNombre}</td>
                  <td>{cVotante.apellidoPaterno}</td>
                  <td>{cVotante.segundoMaterno}</td>
                  <td>{cVotante.edad}</td>
                  <td>{cVotante.telefono}</td>
                  <td>{cVotante.claveElector}</td>
                  <td>{cVotante.seccionElectoral}</td>
                  <td>{cVotante.calle}</td>
                  <td>{cVotante.numExt}</td>
                  <td>{cVotante.colonia}</td>
                  <td>{cVotante.cp}</td>
                  <td>{cVotante.municipio}</td>
                  <td>{cVotante.estado}</td>
                  <td>{cVotante.estatus}</td>
                  <td>{cVotante.borrado}</td>
                  <td>{cVotante.cliente ? <Link to={`c-cliente/${cVotante.cliente.id}`}>{cVotante.cliente.cliente}</Link> : ''}</td>
                  <td>
                    {cVotante.defensorVoto ? (
                      <Link to={`c-defensor-voto/${cVotante.defensorVoto.id}`}>{cVotante.defensorVoto.nombreCompleto}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${cVotante.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button
                        tag={Link}
                        to={`${match.url}/${cVotante.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${cVotante.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No C Votantes found</div>
        )}
      </div>
      {props.totalItems ? (
        <div className={cVotanteList && cVotanteList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ cVotante }: IRootState) => ({
  cVotanteList: cVotante.entities,
  loading: cVotante.loading,
  totalItems: cVotante.totalItems,
});
